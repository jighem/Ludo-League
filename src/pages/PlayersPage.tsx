    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900/80 rounded-3xl p-6 border border-zinc-800/80 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-2xl font-black text-zinc-100 tracking-tight">Player Management</h2>
      ...
      <div className="bg-zinc-900/80 rounded-3xl p-4 border border-zinc-800/80 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white font-medium"
          />
        </div>
          ...
          {(['all', 'active', 'inactive'] as const).map((st) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold capitalize rounded-lg transition-all ${
                statusFilter === st ? 'bg-amber-500 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400'
              }`}
            >
              {st}
            </button>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-slate-400 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <p className="text-base font-bold text-slate-700 dark:text-slate-300">No players registered yet.</p>
          <p className="text-xs text-slate-500">Click "Add Player" above to register your first Ludo player!</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {players.map((p) => (
            <div
              key={p.id}
              className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-xs hover:border-amber-500/40 transition-all flex flex-col justify-between space-y-4"
            >
              <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3
                        ...
                        className="font-bold text-slate-900 dark:text-white hover:text-amber-600 dark:hover:text-amber-400 cursor-pointer transition-colors"
                      >
                        {p.full_name}
                      </h3>
                      {p.nickname && <p className="text-xs text-slate-400 font-medium">"{p.nickname}"</p>}
                    </div>
                  </div>
                ...
                <div className="space-y-1 text-xs text-slate-500 dark:text-slate-400 pt-1">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span>Joined: {p.date_joined}</span>
                  </div>
                </div>
              </div>
