...
export const PlayerProfile: React.FC<PlayerProfileProps> = ({ playerId, onBack }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <button
        onClick={onBack}
        className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-bold text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer shadow-xs"
      >
      ...
      {/* Hero Profile Banner */}
      <div className="bg-white dark:bg-zinc-900/80 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800/80 shadow-md dark:shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-6 transition-colors">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            {player.nickname && <p className="text-xs font-bold text-zinc-500 dark:text-zinc-400">"{player.nickname}"</p>}
            <p className="text-xs text-zinc-500 mt-1 font-medium">Joined {formatDateStr(player.date_joined)}</p>
          </div>
        </div>

        <div className="flex items-center space-x-3 bg-zinc-50 dark:bg-zinc-950/60 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 overflow-x-auto">
          ...
          <div className="text-center px-2">
            ...
            <div className="text-xl font-black text-rose-600 dark:text-rose-400 whitespace-nowrap">
              ⚔️ {totalKills} <span className="text-zinc-400 dark:text-zinc-600">/</span> 💀 {totalDeaths}
            </div>
          </div>
      ...
      <div className="bg-gradient-to-r from-rose-50 dark:from-rose-950/30 via-white dark:via-zinc-900/80 to-amber-50 dark:to-amber-950/20 rounded-3xl p-6 border border-rose-200 dark:border-rose-900/40 shadow-md dark:shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-extrabold px-3 py-1 bg-white dark:bg-zinc-950 rounded-full border border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-300">
            K/D Ratio: <span className="text-amber-600 dark:text-amber-400 font-black">{kdRatio}</span>
          </span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
          <div className="p-4 rounded-2xl bg-white/80 dark:bg-zinc-950/70 border border-rose-200 dark:border-rose-900/30 space-y-1">
            <div className="flex items-center space-x-1.5 text-rose-600 dark:text-rose-400 text-xs font-bold">
              <Crosshair className="w-4 h-4" />
              <span>Total Kills</span>
            </div>
            <div className="text-2xl font-black text-rose-600 dark:text-rose-400">⚔️ {totalKills}</div>
            <p className="text-[10px] text-zinc-500">Tokens captured</p>
          </div>

          <div className="p-4 rounded-2xl bg-white/80 dark:bg-zinc-950/70 border border-zinc-200 dark:border-zinc-800 space-y-1">
            <div className="flex items-center space-x-1.5 text-zinc-600 dark:text-zinc-400 text-xs font-bold">
              <Skull className="w-4 h-4 text-zinc-500" />
              <span>Total Deaths</span>
            </div>
            <div className="text-2xl font-black text-zinc-800 dark:text-zinc-300">💀 {totalDeaths}</div>
          ...
          <div className="p-4 rounded-2xl bg-white/80 dark:bg-zinc-950/70 border border-zinc-200 dark:border-zinc-800 space-y-1">
            ...
            <div className={`text-2xl font-black ${netCombat >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
      ...
      <div className="bg-white dark:bg-zinc-900/80 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800/80 shadow-md dark:shadow-xl space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-extrabold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider flex items-center space-x-2">
            <Flame className="w-4 h-4 text-orange-500 dark:text-orange-400" />
            <span>Recent Form & Combat Action</span>
          </h3>
          <span className="text-xs font-bold text-zinc-500 dark:text-zinc-400">Streak: {summary.currentWinStreak} wins</span>
        </div>
        ...
        {recentMatches.length === 0 ? (
          ...
          <div className="flex flex-wrap items-center gap-2.5 pt-1">
            {recentMatches.map((m, idx) => (
              <div
                key={idx}
                className="flex flex-col items-center justify-center p-2.5 rounded-2xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 min-w-16 shadow-inner"
              ...
              >
                ...
                <span className="text-[9px] font-bold text-zinc-500 dark:text-zinc-400 mt-0.5 flex items-center gap-1">
                  <span className="text-rose-600 dark:text-rose-400 font-extrabold">⚔️{m.kills || 0}</span>
                  <span className="text-zinc-400 dark:text-zinc-500">/</span>
                  <span className="text-zinc-600 dark:text-zinc-300 font-extrabold">💀{m.deaths || 0}</span>
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-zinc-900/80 p-5 rounded-3xl border border-zinc-200 dark:border-zinc-800/80 shadow-md dark:shadow-xl">
        ...
        <div className="bg-white dark:bg-zinc-900/80 p-5 rounded-3xl border border-zinc-200 dark:border-zinc-800/80 shadow-md dark:shadow-xl">
          ...
          <div className="text-2xl font-black text-orange-500 dark:text-orange-400 mt-1">
      ...
      <div className="bg-white dark:bg-zinc-900/80 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800/80 shadow-md dark:shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {([4, 3, 2] as const).map((size) => {
            ...
            return (
              <div key={size} className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800/80 space-y-2.5">
                <div className="flex items-center justify-between font-extrabold text-xs text-zinc-900 dark:text-zinc-100 pb-2 border-b border-zinc-200 dark:border-zinc-800">
                  <span>{size}-Player Games</span>
                  <span className="text-amber-600 dark:text-amber-400">{st.matches} Matches</span>
                </div>

                <div className="grid grid-cols-2 gap-2 text-xs pt-1">
                  <div>
                    <span className="text-zinc-500 text-[10px] font-bold block">Wins</span>
                    <span className="font-bold text-emerald-600 dark:text-emerald-400">{st.wins} ({st.win_pct}%)</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 text-[10px] font-bold block">Avg Score</span>
                    <span className="font-black text-amber-600 dark:text-amber-400">{st.avg_score} pts</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 text-[10px] font-bold block">Combat Kills</span>
                    <span className="font-bold text-rose-600 dark:text-rose-400">⚔️ {st.kills || 0}</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 text-[10px] font-bold block">Deaths</span>
                    <span className="font-bold text-zinc-700 dark:text-zinc-400">💀 {st.deaths || 0}</span>
                  </div>
                </div>
              </div>
            );
        ...
        {monthlyPerformance.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800 text-zinc-500 font-extrabold uppercase tracking-wider text-[10px]">
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60 font-medium text-zinc-700 dark:text-zinc-300">
                {monthlyPerformance.map((m) => (
                  <tr key={m.month} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                    <td className="py-3 px-3 font-bold text-zinc-900 dark:text-zinc-100">{m.month}</td>
                    <td className="py-3 px-3 text-center font-bold">{m.matches}</td>
                    <td className="py-3 px-3 text-center font-bold text-emerald-600 dark:text-emerald-400">{m.wins}</td>
                    <td className="py-3 px-3 text-center">{m.win_pct}%</td>
                    <td className="py-3 px-3 text-center">
                      <span className="font-bold text-rose-600 dark:text-rose-400">⚔️ {m.kills || 0}</span>
                      <span className="text-zinc-400 dark:text-zinc-600 mx-1">/</span>
                      <span className="font-bold text-zinc-600 dark:text-zinc-400">💀 {m.deaths || 0}</span>
                    </td>
                    <td className="py-3 px-3 text-right font-black text-amber-600 dark:text-amber-400">{m.average_score.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
